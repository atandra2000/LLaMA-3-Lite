...
The old `CODE_MAP.md` was generated by `scripts/generate_code_map.py` from
the docs' own `<module>.py:<symbol>` citations; the map below is the
hand-maintained successor. `tests/test_doc_refs.py` (CI) verifies every
citation resolves.
